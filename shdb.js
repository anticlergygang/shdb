...
const cipherDirPromise = (directory, key, salt = 'salt', iv = Buffer.alloc(16, 0), timeout = 10000) => new Promise((resolve, reject) => {
    try {
        let cipherDirPromiseTimeout = setTimeout(() => {
            reject('minor cipherDirPromise timeout')
        }, timeout)
                ...
                if (fileKey.indexOf('.DS_Store') === -1) {
                    cipherFile(fileKey, password, salt, iv).then(output => {
                        ...
                        if (count >= files.files.length) {
                            clearTimeout(cipherDirPromiseTimeout)
                            resolve('finished')
                        }
                    }).catch(err => {
                        reject(err)
        ...
        reject(`major cipherDirPromise error \n${util.inspect(err)}`)
    }
})
